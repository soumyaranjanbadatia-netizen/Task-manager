  ...
  const saveProfile = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        ...
        { name: profile.name, email: profile.name },
        { headers: { Authorization: `Bearer${token}` } }
      );
      if (data.success) {
        setCurrentUser((prev) => ({
  ...
  const changePassword = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        ...
        {
          currntPassword: passwords.current,
          newPassword: passwords.new,
        },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (data.success) {
        toast.success("passwrod changes");
      ...
      <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm">
              Manage you profile and security settings
            </p>
          </div>
        </div>
        <div className="grid md:grid-cols-2 gap-8">
          <section className={SECTION_WRAPPER}>
            ...
            <form onSubmit={saveProfile} className="sapce-y-4">
          ...
          <section className={SECTION_WRAPPER}>
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-xl font-semibold to-gray-800">Security</h2>
            </div>
            <form onSubmit={changePassword} className="spaxe-y-4">
